...
||| A `Stream_ c f es o` is a newtype wrapper around a `Pull_ c f o es ()`.
|||
...
||| ```
public export
record Stream_ c f es o where
  constructor S
  pull : Pull_ c f o es ()

||| Alias for a `Stream_` operating on lists of values.
public export
0 Stream : (List Type -> Type -> Type) -> List Type -> Type -> Type
Stream = Stream_ List

||| Alias for a `Stream_` that cannot emit any output.
public export
0 EmptyStream : (List Type -> Type -> Type) -> List Type -> Type
EmptyStream f es = Stream_ (const Void) f es ()

||| Alias for a `Stream_` that emits single values.
public export
0 Stream1 : (List Type -> Type -> Type) -> List Type -> Type -> Type
Stream1 = Stream_ Prelude.id
...
export %inline
MErr (Stream1 f) where
  succeed      = S . outputChunk
  fail         = S . fail
  bind (S v) f = S $ bindOutput (pull . f) v
  attempt      = S . attemptOutput1 . pull
  mapImpl f p  = S (mapChunk f p.pull)
  appImpl f p  = bind f (`mapImpl` p)

export %inline
Semigroup (Stream_ c f es o) where
  S p <+> S q = S $ p >> q

export %inline
Monoid (Stream_ c f es o) where
  neutral = S (pure ())
...
export %inline
ELift1 s f => ELift1 s (Stream1 f) where
  elift1 act = S (elift1 act >>= outputChunk)

export %inline
ELift1 World f => HasIO (Stream1 f es) where
  liftIO act = lift1 (ioToF1 act)
...
||| Returns a stream that emits a single chunk of values.
export %inline
emitChunk : c o -> Stream_ c f es o
emitChunk = S . outputChunk
...
||| Creates an empty stream that just evaluates the supplied effect.
||| If the effect fails, the returned stream fails.
export %inline
exec : f es () -> Stream_ c f es o
exec = S . Eval

||| Like `eval` but emits the result as a single chunk
export %inline
evalChunk : f es (c o) -> Stream_ c f es o
evalChunk act = S (Eval act >>= outputChunk)
...
||| it returns `Nothing`.
export %inline
unfoldEvalChunk : f es (Maybe $ c o) -> Stream_ c f es o
unfoldEvalChunk act = S $ unfoldEvalChunkMaybe act

||| Infinitely repeats the given stream.
export
repeat : Stream_ c f es o -> Stream_ c f es o
...
export
force : f es (Stream_ c f es o) -> Stream_ c f es o
force fun =
  S $ do
    S p <- Eval fun
    p
...
||| Wraps a `Pull` and runs it in a new child scope.
export %inline
stream : Pull_ c f o es () -> Stream_ c f es o
stream p = S (OScope None p)

||| Runs the given stream in a new child scope.
export %inline
scope : Stream_ c f es o -> Stream_ c f es o
scope = stream . pull
...
export %inline
consChunk : c o -> Stream_ c f es o -> Stream_ c f es o
consChunk vs = S . consChunk vs . pull
...
||| Alias for `(<+>)`
export %inline
(++) : Stream_ c f es o -> Stream_ c f es o -> Stream_ c f es o
(++) = (<+>)
...
export %inline
foreachChunk : (c o -> f es ()) -> Stream_ c f es o -> EmptyStream f es
foreachChunk f = S . sinkChunk f . pull

||| Empties a stream silently dropping all output.
export %inline
drain : Applicative (f es) => Stream_ c f es o -> EmptyStream f es
drain = foreachChunk (\_ => pure ())
...
||| otherwise affecting the emitted values.
export
observeChunk : (c o -> f es ()) -> Stream_ c f es o -> Stream_ c f es o
observeChunk act = S . observeOutput act . pull
...
export
finallyWeak : Applicative (f es) => f [] () -> Stream_ c f es o -> Stream_ c f es o
finallyWeak cleanup = bracketWeak (pure ()) (const cleanup) . const

||| Makes sure the given cleanup action is run once the stream
||| terminates.
export
finally : Applicative (f es) => f [] () -> Stream_ c f es o -> Stream_ c f es o
