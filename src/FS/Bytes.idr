...
export
breakNL : Pull f ByteString es r -> Pull f ByteString es r
breakNL = scanFull empty breakLastNL nonEmpty
...
splitImpl k bs =
  case splitAt k bs of
    Nothing         => All (k `minus` size bs)
    Just (pre,post) =>
      let Just y := nebs post | Nothing => All (k `minus` size pre)
       in Middle pre y
