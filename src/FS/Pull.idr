...
||| A `Pull s f o es r` is a - possibly infinite - series of effectful
...
||| of type `r`. The `s` phantom type corresponds to the state thread the
||| pull will be run in. If this is universally quantified, the pull can
||| be run as a pure computation. Otherwise - if the pull should be run in
||| an `IO` type - `s` will be set to `World`. See the ref1 library to get
||| an introduction about state threads.
|||
...
|||
||| ```idris
||| Bind : Pull s f o es r -> Inf (r -> Pull s f o es s) -> Pull s f o es s
||| ```
|||
...
data Pull :
  ...
  Att    : Pull s f o es r -> Pull s f o fs (Result es r)

  ||| Sequencing of `Pull`s via monadic bind `(>>=)`.
  Bind   : Pull s f o es r -> (r -> Pull s f o es t) -> Pull s f o es t
  ...
  Acquire : f es r -> (r -> f [] ()) -> Pull s f o es r
  ...
  WScope : Pull s f o es r -> (inner, outer : ScopeID) -> Pull s f o es r

  ||| Internal: A pull for returning the current scope
  GScope : Pull s f o es (Scope s f)
...
export %inline
ELift1 s f => ELift1 s (Pull s f o) where
  elift1 = Eval . elift1
...
export %inline
output1 : o -> Pull s f o es ()
output1 v = Output [v]

||| Emits a list of values
export %inline
output : List o -> Pull s f o es ()
output [] = pure ()
output vs = Output vs

||| Emits values from an arbitrary foldable
export %inline
foldable : Foldable m => m o -> Pull s f o es ()
...
export
fromChunks : List (List o) -> Pull s f o es ()
fromChunks vss =
...
export
unfoldEval : f es (Either r o) -> Pull s f o es r
unfoldEval act =
...
export
unfoldEvalMaybe : f es (Maybe o) -> Pull s f o es ()
unfoldEvalMaybe act =
  assert_total $ Eval act >>= \case
    Nothing => pure ()
...
export
unfoldEvalChunk : f es (List o, Maybe r) -> Pull s f o es r
unfoldEvalChunk act =
  assert_total $ Eval act >>= \case
...
export
unfoldEvalChunkMaybe : f es (Maybe $ List o) -> Pull s f o es ()
...
||| new combinators and topologies.
export %inline
inScope : Scope s f -> Pull s f o es r -> Pull s f o es r
inScope = IScope

||| Acquires a resource that will be released once the current
||| scope is cleaned up.
export %inline
acquire : (acq : f es r) -> (release : r -> f [] ()) -> Pull s f o es r
acquire = Acquire
...
export %inline
cons : List o -> Pull s f o es r -> Pull s f o es r
cons [] p = p
...
export
uncons1 : Pull s f o es r -> Pull s f q es (Either r (o, Pull s f o es r))
uncons1 p =
...
export
stepLeg : StepLeg s f es o -> Pull s f q es (Maybe (List o, StepLeg s f es o))
stepLeg (SL p sc) =
  inScope sc $ do
    uncons p >>= \case
      Left _       => pure Nothing
...
||| its desired scope.
export
endLeg : List o -> StepLeg s f es o -> Pull s f o es ()
...
unconsLimit :
  ...
  -> {auto 0 p : IsSucc n}
  -> Pull s f o es ()
  -> Pull s f q es (Maybe (List o, Pull s f o es ()))
unconsLimit n p =
  uncons p >>= \case
    Left _       => pure Nothing
    Right (os,q) =>
...
||| remainder.
export
take : Nat -> Pull s f o es () -> Pull s f o es (Maybe $ Pull s f o es ())
...
export
last : Pull s f o es () -> Pull s f q es (Maybe o)
last = go Nothing
  where
    go : Maybe o -> Pull s f o es () -> Pull s f q es (Maybe o)
    go m p =
      assert_total $ uncons p >>= \case
        Left _         => pure m
        Right ([],q)   => go m q
...
export
peek : Pull s f o es () -> Pull s f q es (Maybe (List o, Pull s f o es ()))
peek p =
  uncons p >>= \case
    Left _       => pure Nothing
...
export
peek1 : Pull s f o es () -> Pull s f q es (Maybe (o, Pull s f o es ()))
peek1 p =
  assert_total $ uncons p >>= \case
    Left _              => pure Nothing
...
export
drop : Nat -> Pull s f o es () -> Pull s f o es (Maybe $ Pull s f o es ())
...
takeRight :
  ...
  -> {auto 0 p : IsSucc n}
  -> Pull s f o es ()
  -> Pull s f q es (List o)
takeRight n = go []
  where
    go : List o -> Pull s f o es () -> Pull s f q es (List o)
    go xs p =
...
dropWhile_ :
  ...
  -> (o -> Bool)
  -> Pull s f o es ()
  -> Pull s f o es (Maybe $ Pull s f o es ())
...
find :
     (o -> Bool)
  -> Pull s f o es ()
  -> Pull s f p es (Maybe (o, Pull s f o es ()))
find pred p =
  assert_total $ uncons p >>= \case
    Left _       => pure Nothing
...
export
mapChunks : (List o -> List p) -> Pull s f o es r -> Pull s f p es r
mapChunks f p =
  assert_total $ uncons p >>= \case
    Left x      => pure x
...
export %inline
fold : p -> (p -> o -> p) -> Pull s f o es () -> Pull s f q es p
...
||| values.
export %inline
fold1 : (o -> o -> o) -> Pull s f o es () -> Pull s f q es (Maybe o)
fold1 g p =
  assert_total $ uncons p >>= \case
    Left _         => pure Nothing
...
||| the given predicate
export
all : (o -> Bool) -> Pull s f o es () -> Pull s f q es Bool
all pred p =
  assert_total $ uncons p >>= \case
    Left _ => pure True
...
||| the given predicate
export
any : (o -> Bool) -> Pull s f o es () -> Pull s f q es Bool
any pred p =
  assert_total $ uncons p >>= \case
    Left _ => pure False
...
export
unconsBind : Pull s f o es () -> (List o -> Pull s f p es ()) -> Pull s f p es ()
unconsBind p g =
  assert_total $ uncons p >>= \case
    Left _       => pure ()
...
export
bindOutput : (List o -> Pull s f p es ()) -> Pull s f o es () -> Pull s f p es ()
bindOutput f p =
  assert_total $ uncons p >>= \case
    Left _      => pure ()
...
export
bindOutput1 : (o -> Pull s f p es ()) -> Pull s f o es () -> Pull s f p es ()
bindOutput1 f p =
...
export
attemptOutput : Pull s f o es () -> Pull s f (Result es o) fs ()
attemptOutput p =
...
data StepRes :
  ...
  Done : (ss : Scope s f) -> (res : r) -> StepRes s f o es r
